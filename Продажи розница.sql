...
T1._Fld5972 AS discount, 
MONTH(DATEADD(year, -2000, T1._Period)) AS month_, -- месяц
YEAR(DATEADD(year, -2000, T1._Period)) AS year_,   -- год
...
-- Без самовывозов только розничные магазины без ОМСК-Дискаунтер

SELECT 
Code, 
product, 
COUNT(DISTINCT(month_)) AS count_month, 
COUNT(DISTINCT(stock)) AS count_stock, 
SUM(amount) AS sum_amount, 
SUM(price) AS sum_price, 
SUM(sales) AS sum_sales, 
SUM(sales) - SUM(price) AS profit 
FROM #SalesNoPickups 
